...
#include <xdiag/matrix/spinhalf/matrix_generic.hpp>
...
void apply(OpSum const &ops, basis_t const &basis_in, mat_t const &mat_in,
  ...
  spinhalf::matrix_generic<coeff_t>(
      ops, basis_in, basis_out,
      [&](int64_t idx_in, int64_t idx_out, coeff_t val) {
        matrix::fill_apply(mat_in, mat_out, idx_in, idx_out, val);
      });
}
XDIAG_CATCH
...
INSTANTIATE_XDIAG_MATRIX_SPINHALF_APPLY_MATRICES(BasisOnTheFly, Subsets,
                                                 uint32_t);
...
INSTANTIATE_XDIAG_MATRIX_SPINHALF_APPLY_MATRICES(BasisOnTheFly, Combinations,
                                                 uint32_t);
// END_INSTANTIATION_GROUP
...
INSTANTIATE_XDIAG_MATRIX_SPINHALF_APPLY_MATRICES(BasisOnTheFly, LinTable,
                                                 uint32_t);
// END_INSTANTIATION_GROUP
